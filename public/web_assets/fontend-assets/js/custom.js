...
(function($) {
    ...
    function setup() {
            ...
            success: function (data) {
                ...
                $.each(object,function (index, value) {
                    $('#video_gallery').append(
                        ...
                        '                            <iframe width="560" height="315" src="https://www.youtube.com/embed/'+value.id.videoId+'" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>\n' +
                        '                        </div>\n' +
        ...
        $.ajax({
            url: '/get_footer_address',
            method: 'get',
            success: function (response) {
                // console.log(response)
                let data = JSON.parse(response);
                $('#footer_eiin').text(data.ein_no);
                $('#footer_school_code').text(data.school_code);
                $('#footer_address').text(data.address);
            },
            error: function (resp) {
                console.log(resp);
            }
        ...
        $.ajax({
            url: '/get_footer_contact',
            method: 'get',
            success: function (response) {
                let data = JSON.parse(response);
                $('#footer_phone').text(data.phone)
                $('#footer_email').text(data.email)
                $('#footer_website').text(data.website)
                //console.log(response)
            },
            error: function (resp) {
                console.log(resp);
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $.each(data, function (index, value) {
                    let icon = value.name.toLowerCase();
                    $('#footer_social_link').append(
                        '<li><a href="'+value.social_link+'" target="_blank"><i class="fa fa-'+icon+'" title='+value.name.toLowerCase()+'></i></a></li>'
                        // '<li><a href="'+value.social_link+'" target="_blank"><img src="'+value.social_icon+'" alt=""></a></li>'
                    )
                })
            },
            error: function (resp) {
